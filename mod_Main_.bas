        ...
        If vbc.Type = 1 Or vbc.Type = 2 Or vbc.Type = 100 Then
            If vbc.CodeModule.CountOfLines > 0 Then
                ...
                For i = 1 To vbc.CodeModule.CountOfLines
                    ...
                    ' Insertar bloque de ruido entre procedimientos:
                    ' Solo cuando la linea actual es End Sub/Function/Property
                    ' y la siguiente no es otro End ni esta vacia (hay mas codigo)
                    Dim linTrim As String
                    linTrim = Trim(linea)
                    If linTrim Like "End Sub*" Or _
                       linTrim Like "End Function*" Or _
                       linTrim Like "End Property*" Then
                        ' 40% de probabilidad de insertar bloque
                        If Rnd < 0.4 Then
                            nuevoCodigo = nuevoCodigo & vbCrLf & _
                                          mod_Garbage.GenerarBloqueRuido() & _
                                          vbCrLf
                        End If
                    End If
